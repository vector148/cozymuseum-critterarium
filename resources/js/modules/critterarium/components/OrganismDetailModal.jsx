...
function InfoCell({ label, value, italic = false, alwaysRender = false }) {
  if (!value && !alwaysRender) return null;
  return (
    <div className="cell">
      <div className="k">{label}</div>
      <div className="v">{value ? (italic ? <i>{value}</i> : value) : ""}</div>
    </div>
  );
}
    ...
    <div className="modal" role="dialog" aria-modal="true" aria-label={item.displayName}>
      ...
      <div className="modal-content glass-card deep organism-detail" onClick={(event) => event.stopPropagation()}>
        ...
        <div className="detail-hero organism-detail-hero">
          {media.src ? (
            <OptimizedImage
              className="organism-detail-image"
              src={media.src}
          ...
          <div className="organism-detail-image-fallback" style={media.src ? { display: "none" } : undefined} aria-hidden="true">◇</div>
          <div className="big-title">{item.displayName}{item.isDangerous ? " ☠️" : ""}</div>
        ...
        <div className="info-grid detail-info-grid">
          <InfoCell label={t(locale, "kingdom")} value={item.displayKingdom} alwaysRender={true} />
          <InfoCell label={t(locale, "phylum")} value={item.displayPhylum} alwaysRender={true} />
          <InfoCell label={t(locale, "className")} value={item.className} alwaysRender={true} />
          <InfoCell label={t(locale, "order")} value={item.order} alwaysRender={true} />
          <InfoCell label={t(locale, "family")} value={item.family} alwaysRender={true} />
          <InfoCell label={t(locale, "genus")} value={item.genus} alwaysRender={true} />
          <InfoCell label={t(locale, "species")} value={item.rank === "SPECIES" || item.rank === "SUBSPECIES" ? item.scientificName : ""} italic={true} alwaysRender={true} />
          <InfoCell label={t(locale, "lifeState")} value={item.lifeState === "extinct" ? t(locale, "extinctStatus") : t(locale, "extantStatus")} alwaysRender={true} />
        ...
        {item.displayDescription ? <p className="organism-description">{item.displayDescription}</p> : null}
