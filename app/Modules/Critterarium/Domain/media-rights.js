...
export const RIGHTS_FREE_LICENSES = Object.freeze(["CC0", "Public Domain", "Unsplash License", "Pexels License", "Pixabay License"]);
...
export function normalizeRightsFreeLicense(value) {
  ...
  if (/^pexels(?:\s|$)/.test(normalized)) return "Pexels License";
  if (/^pixabay(?:\s|$)/.test(normalized)) return "Pixabay License";
  return "";
}
...
export function imageRightsFailures(row = {}) {
  ...
  if (!isRightsFreeLicense(row.imageLicense)) failures.push("image license is not CC0, Public Domain, Unsplash, Pexels, or Pixabay License");
