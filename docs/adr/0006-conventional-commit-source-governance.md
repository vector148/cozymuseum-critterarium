...
- Keep exactly one consolidated commit per calendar day, using `Asia/Saigon` as the day boundary (Maximum 1 commit per day). It is strictly forbidden to squash multiple days' work into a single commit, or to create multiple commits in a single day. Every additional code, data, documentation, QA, or deployment fix made on the same day must amend that day's commit.
